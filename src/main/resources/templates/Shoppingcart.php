...
if (isset($_SESSION['loginuser'])) {
    ...
    if (isset($_GET["delete"])) {
        $stmt = $conn->prepare("DELETE FROM `cart` WHERE `custid` = ? AND `productname` = ?");
        $stmt->bind_param("ss", $custid, $_GET["productname"]);
        if ($stmt->execute()) {
            echo '<script>alert("Product deleted from cart");</script>';
        } else {
            echo '<script>alert("Cannot delete product from cart");</script>';
        }
    }

    if (isset($_POST["update"])) {
        foreach ($_POST["update"] as $productname => $value) {
            $new_quantity = $_POST["new_quantity"][$productname];
            if (ctype_digit(strval($new_quantity)) && $new_quantity > 0 && $new_quantity <= 5) {
                $stmt = $conn->prepare("UPDATE `cart` SET `quantity` = ? WHERE `custid` = ? AND `productname` = ?");
                $stmt->bind_param("sss", $new_quantity, $custid, $productname);
                if ($stmt->execute()) {
                    echo '<script>alert("Quantity for ' . $productname . ' updated successfully");</script>';
                } else {
                    echo '<script>alert("Cannot update quantity for ' . $productname . '");</script>';
                }
            } else {
                echo '<script>alert("Invalid quantity entered for ' . $productname . '");</script>';
            }
        }
    }


    $stmt = $conn->prepare("SELECT `productname`, `price`, `quantity`, `productimage`, `quantity` * `price` AS `sum` FROM `cart` WHERE custid = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        echo '<script>window.location.href="Emptycart.html";</script>';
    }
} else {
    echo '<script>alert("Sign in first");window.location.href="login.php";</script>';
}
?>
<!DOCTYPE html>
<html lang="en">
...
<a class="a1" href="Bookproduct.html"><p class="p1">Continue shopping</p></a>
<div class=header>
    <p class="p2">PRODUCT</p>
    <div class="subh">
        <p>PRICE</p>
        <p for="new-quantity">QUANTITY</p>
        <p>SUM</p>
    </div>
</div>
    ...
    foreach ($data as $product):
        ...
        <div class="Product1">
            <div class="Product-name-info">
                <img class="Product-img" src="<?php echo htmlspecialchars($product['productimage']); ?>">
                <div class="Product-name">
                    <div class="title"><span><?php echo htmlspecialchars($product['productname']); ?></span></div>
                    <a href="?delete=1&productname=<?php echo urlencode($product['productname']); ?>"
                       class="p1">deletion</a>
                </div>
            </div>
            <div class="Product-price-info">
                ...
                <div class="quantity">
                    <input id="new-quantity-<?php echo htmlspecialchars($product['productname']); ?>"
                           name="new_quantity[<?php echo htmlspecialchars($product['productname']); ?>]" type="text"
                           value="<?php echo htmlspecialchars($product['quantity']); ?>">
                </div>
                <div class="sum">￥<span><?php echo htmlspecialchars($product['sum']); ?></span></div>
                <input type="hidden" name="productname"
                       value="<?php echo htmlspecialchars($product['productname']); ?>">
                <button name="update[<?php echo htmlspecialchars($product['productname']); ?>]" type="submit"
...
<script>
    ...
    document.querySelectorAll('input[name^=new_quantity]').forEach(function (input) {
        input.addEventListener('keydown', function (event) {
            if (event.key === 'Enter') {
                event.preventDefault(); // Prevent the form from submitting
                var productName = this.getAttribute('name').split('[')[1].split(']')[0];
                document.querySelector('button[name="update[' + productName + ']"]').click();
            }
        });
    });
</script>
