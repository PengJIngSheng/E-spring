...
$isInputValid = true;
$isCartValid = true; // Flag to indicate if cart retrieval is successful

if ($_SERVER["REQUEST_METHOD"] == 'POST' && isset($_POST["submit"]) && isset($_SESSION['loginuser'])) {
    $trimmed = array_map('trim', $_POST);
    $custid = $_SESSION['loginuser']['custid'];
    $card = $date = $cvv = '';

    if (!preg_match('/^\d{13,19}$/', $trimmed['number'])) {
        echo '<script>alert("Please enter a valid card number!");</script>';
        $isInputValid = false;
    } else {
        $card = $trimmed['number'];
    }

    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $trimmed['date']) || strtotime($trimmed['date']) < time()) {
        echo '<script>alert("Please enter a valid expiration date!");</script>';
        $isInputValid = false;
    } else {
        $date = $trimmed['date'];
    }

    if (!preg_match('/^\d{3,4}$/', $trimmed['cvv'])) {
    ...
    if ($isInputValid) {
        $query = "SELECT `email`, `title`, `firstname` FROM `customerdb` WHERE `custid` = ?";
        $stmt = $conn->prepare($query);

        if ($stmt) {
            $stmt->bind_param("s", $custid); // Bind the custid parameter
            $stmt->execute();
            $stmt->bind_result($email, $title, $firstname);
            $stmt->fetch();
            $stmt->close();
            ...
            $queryCart = "SELECT `productname`, `price`, `quantity` FROM `cart` WHERE `custid` = ?";
            $stmtCart = $conn->prepare($queryCart);

            if ($stmtCart) {
                $stmtCart->bind_param("s", $custid);
                $stmtCart->execute();
                $stmtCart->bind_result($productname, $price, $quantity);

                $cartItems = [];
                $totalPrice = 0;

                while ($stmtCart->fetch()) {
                ...
                // Call the sendRecipt function with user information, cart items, and total price
                $flag = sendRecipt($to, $title, $firstname, $card, $date, $cvv, $cartItems, $totalPrice);

                if ($flag) {
                    echo '<script>alert("Payment Successful");</script>';
                    header('Location: EmptyPayment.php');
                } else {
                    echo '<script>alert("Payment failed");</script>';
                }
            } else {
                echo "Failed to retrieve cart items.";
                $isCartValid = false;
            }
        } else {
...
<body>
<form action="Cardpay.php" method="post" novalidate="novalidate">
    <div class="Banner">
        <div class="Nav">
    ...
    <div class="Text1">
        <p>
            Fill in all fields with contain*
        </p>
    </div>
    ...
    <div class="Card-input">
        <input type="text" name="number" id="cardNumber"
               value="<?php if (isset($trimmed['number'])) echo $trimmed['number']; ?>"/>
    </div>
    <div class="Date-label">
        <p>Expired Date*</p>
    </div>
    <div class="Date-input">
        <input type="date" name="date" id="expirationDate"
               value="<?php if (isset($trimmed['date'])) echo $trimmed['date']; ?>"/>
    </div>
    <div class="Cvv-label">
        <p>CVV*</p>
    </div>
    <div class="Cvv-input">
        <input type="text" name="cvv" id="cvv"
               value="<?php if (isset($trimmed['cvv'])) echo $trimmed['cvv']; ?>"/>
    </div>
    <div class="Text2">
        <p>
            All fields mark with an * are mandatory
        </p>
    </div>
    <div class="Submit-button">
        <span class="mas">PAY</span>
        <button id="work" type="submit" name="submit" value="PAY">PAY</button>
    </div>
    <div class="Bottom-Banner">
        <div class="img-container">
            <a href="Mainpage.html"><img src="Logo.png" alt=""></a>
        </div>
        <div class="Nav">
            <img src="footersign.png" alt="">
            <a href=""><p class="p1">TERMS OF USE</p></a>
            <a href=""><p class="p2">ABOUT US</p></a>
            <a href=""><p class="p3">PRIVACY POLICY</p></a>
            <a href="https://www.instagram.com/shanmu_cafe/">
                <svg width="37" height="37" viewBox="0 0 37 37" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M27.5714 2.75H9.57141C5.84349 2.75 2.82141 5.77208 2.82141 9.5V27.5C2.82141 31.2279 5.84349 34.25 9.57141 34.25H27.5714C31.2993 34.25 34.3214 31.2279 34.3214 27.5V9.5C34.3214 5.77208 31.2993 2.75 27.5714 2.75Z"
                          fill="url(#paint0_radial_193_72)"/>
                    <path d="M27.5714 2.75H9.57141C5.84349 2.75 2.82141 5.77208 2.82141 9.5V27.5C2.82141 31.2279 5.84349 34.25 9.57141 34.25H27.5714C31.2993 34.25 34.3214 31.2279 34.3214 27.5V9.5C34.3214 5.77208 31.2993 2.75 27.5714 2.75Z"
                          fill="url(#paint1_radial_193_72)"/>
                    <path d="M27.5714 2.75H9.57141C5.84349 2.75 2.82141 5.77208 2.82141 9.5V27.5C2.82141 31.2279 5.84349 34.25 9.57141 34.25H27.5714C31.2993 34.25 34.3214 31.2279 34.3214 27.5V9.5C34.3214 5.77208 31.2993 2.75 27.5714 2.75Z"
                          fill="url(#paint2_radial_193_72)"/>
                    <path d="M26.4464 12.3125C26.4464 13.2445 25.6909 14 24.7589 14C23.827 14 23.0714 13.2445 23.0714 12.3125C23.0714 11.3805 23.827 10.625 24.7589 10.625C25.6909 10.625 26.4464 11.3805 26.4464 12.3125Z"
                          fill="white"/>
                    <path fill-rule="evenodd" clip-rule="evenodd"
                          d="M18.5714 24.125C21.678 24.125 24.1964 21.6066 24.1964 18.5C24.1964 15.3934 21.678 12.875 18.5714 12.875C15.4648 12.875 12.9464 15.3934 12.9464 18.5C12.9464 21.6066 15.4648 24.125 18.5714 24.125ZM18.5714 21.875C20.4354 21.875 21.9464 20.364 21.9464 18.5C21.9464 16.636 20.4354 15.125 18.5714 15.125C16.7074 15.125 15.1964 16.636 15.1964 18.5C15.1964 20.364 16.7074 21.875 18.5714 21.875Z"
                          fill="white"/>
                    <path fill-rule="evenodd" clip-rule="evenodd"
                          d="M7.32141 18.05C7.32141 14.2697 7.32141 12.3794 8.05712 10.9356C8.70426 9.66547 9.73688 8.63285 11.007 7.98571C12.4508 7.25 14.3411 7.25 18.1214 7.25H19.0214C22.8017 7.25 24.692 7.25 26.1358 7.98571C27.4059 8.63285 28.4386 9.66547 29.0857 10.9356C29.8214 12.3794 29.8214 14.2697 29.8214 18.05V18.95C29.8214 22.7303 29.8214 24.6206 29.0857 26.0644C28.4386 27.3345 27.4059 28.3672 26.1358 29.0143C24.692 29.75 22.8017 29.75 19.0214 29.75H18.1214C14.3411 29.75 12.4508 29.75 11.007 29.0143C9.73688 28.3672 8.70426 27.3345 8.05712 26.0644C7.32141 24.6206 7.32141 22.7303 7.32141 18.95V18.05ZM18.1214 9.5H19.0214C20.9488 9.5 22.2588 9.50175 23.2715 9.58449C24.2581 9.66508 24.7625 9.81116 25.1144 9.99047C25.9611 10.4219 26.6495 11.1103 27.0809 11.957C27.2602 12.3089 27.4064 12.8133 27.4869 13.7999C27.5696 14.8126 27.5714 16.1227 27.5714 18.05V18.95C27.5714 20.8773 27.5696 22.1874 27.4869 23.2001C27.4064 24.1867 27.2602 24.6911 27.0809 25.043C26.6495 25.8897 25.9611 26.5781 25.1144 27.0095C24.7625 27.1888 24.2581 27.335 23.2715 27.4155C22.2588 27.4982 20.9488 27.5 19.0214 27.5H18.1214C16.1941 27.5 14.884 27.4982 13.8713 27.4155C12.8848 27.335 12.3803 27.1888 12.0284 27.0095C11.1817 26.5781 10.4933 25.8897 10.0619 25.043C9.88258 24.6911 9.73649 24.1867 9.6559 23.2001C9.57317 22.1874 9.57141 20.8773 9.57141 18.95V18.05C9.57141 16.1227 9.57317 14.8126 9.6559 13.7999C9.73649 12.8133 9.88258 12.3089 10.0619 11.957C10.4933 11.1103 11.1817 10.4219 12.0284 9.99047C12.3803 9.81116 12.8848 9.66508 13.8713 9.58449C14.884 9.50175 16.1941 9.5 18.1214 9.5Z"
                          fill="white"/>
                    <defs>
                        <radialGradient id="paint0_radial_193_72" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse"
                                        gradientTransform="translate(14.0714 26.375) rotate(-55.3758) scale(28.7096)">
                            <stop stop-color="#B13589"/>
                            <stop offset="0.79309" stop-color="#C62F94"/>
                            <stop offset="1" stop-color="#8A3AC8"/>
                        </radialGradient>
                        <radialGradient id="paint1_radial_193_72" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse"
                                        gradientTransform="translate(12.9464 35.375) rotate(-65.1363) scale(25.4185)">
                            <stop stop-color="#E0E8B7"/>
                            <stop offset="0.444662" stop-color="#FB8A2E"/>
                            <stop offset="0.71474" stop-color="#E2425C"/>
                            <stop offset="1" stop-color="#E2425C" stop-opacity="0"/>
                        </radialGradient>
                        <radialGradient id="paint2_radial_193_72" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse"
                                        gradientTransform="translate(1.13391 3.875) rotate(-8.1301) scale(43.7523 9.35815)">
            ...
            <a href="https://www.twitter.com">
                <svg width="41" height="31" viewBox="0 0 41 31" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M13.1267 31C8.50533 31 4.19737 29.7318 0.571411 27.5436C3.64997 27.7317 9.08294 27.2815 12.4623 24.2404C7.37867 24.0203 5.08607 20.3418 4.78704 18.7698C5.21897 18.927 7.27898 19.1157 8.4419 18.6755C2.59413 17.2921 1.69703 12.4503 1.89638 10.9727C2.99284 11.6958 4.85348 11.9473 5.58445 11.8843C0.135397 8.20584 2.09574 2.67242 3.05928 1.47769C6.96973 6.58908 12.8303 9.45985 20.0806 9.61944C19.9438 9.05389 19.8717 8.46489 19.8717 7.85998C19.8717 3.51907 23.5906 0 28.1781 0C30.575 0 32.7348 0.960663 34.251 2.49731C35.8527 2.14319 38.2633 1.31422 39.4417 0.597357C38.8477 2.60954 36.9984 4.28809 35.8798 4.91024C35.8707 4.88898 35.8891 4.9314 35.8798 4.91024C36.8624 4.77001 39.5211 4.28793 40.5714 3.61562C40.052 4.74603 38.0914 6.62555 36.4824 7.67776C36.7818 20.134 26.6807 31 13.1267 31Z"
                          fill="#47ACDF"/>
                </svg>
            </a>
        </div>
...
<script>
    window.onload = function () {
        var letters = document.querySelectorAll('.Coffee-Words p');
